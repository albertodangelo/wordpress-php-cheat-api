...
const restRoot = "http://localhost:8001";
...
function updateTitle(postID, newTitle) {
  let restURL = `${restRoot}/index.php/wp-json/wp/v2/posts/${postID}`;
  ...
  const response = fetch(restURL, {
    ...
    .then((response) => {
      ...
      document.querySelector(
        `.navigation-list a[data-id="${postID}"]`
      ).innerHTML = response.title.rendered;
    })
    .catch((error) => {
...
function getToken(username, password) {
  let restURL = `${restRoot}/index.php/wp-json/jwt-auth/v1/token`;
