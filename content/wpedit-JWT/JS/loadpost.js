...
const sourceURL = "http://localhost:8001/index.php";
...
function buildPost(postObject) {
  ...
  document.querySelector(".featured-image")
    ? (featImgElement = document.querySelector(".featured-image"))
    : (featImgElement = false);
  ...
  document.querySelector(".post-author").innerHTML =
    postObject._embedded.author[0].name;
  document.querySelector(".post-date").innerHTML = date.toDateString();
  document.querySelector(
    ".post-link"
  ).innerHTML = `<span class="post-link"><a href="${postObject.link}" rel="bookmark">Original source</a></span>`;
  document.querySelector(
    ".post-content"
  ).innerHTML = `<div class="post-content">${postObject.content.rendered}</div>`;
