...
function getDate(task) {
  ...
  let date = `<div class="task-date">Task created <time>${taskDate.toLocaleDateString(
    "en-US",
    options
  )}</time></div>`;
  ...
  if (task.date != task.modified) {
    modified = `<div class="task-date">Task updated <time>${modifiedDate.toLocaleDateString(
      "en-US",
      options
    )}</time></div>`;
  }
    ...
    outcome = `
            <div class="task-outcome">
                ...
                <div class="task-block">
                    ${taskObject.cmb2.taskbox_rest_metabox.taskbook_outcome}
                </div>
                <h3 class="task-sub">Post-task stress level</h3>
                <div class="task-pre-level level">
                    ${getLevel(
                      taskObject.cmb2.taskbox_rest_metabox.taskbook_post_level
                    )}
                </div>
            </div>`;
  } else {
...
function buildTask(taskObject) {
  ...
  taskArticle.innerHTML = `
        ...
        <div class="task-prediction">
            ...
            <div class="task-block">
                ${taskObject.cmb2.taskbox_rest_metabox.taskbook_prediction}
            </div>
            <h3 class="task-sub">Pre-task stress level</h3>
            <div class="task-pre-level level">
                ${getLevel(
                  taskObject.cmb2.taskbox_rest_metabox.taskbook_pre_level
                )}
            </div>
        </div><!-- .task-prediction -->
