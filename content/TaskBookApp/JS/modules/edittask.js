...
function generateFormData(newTask) {
  ...
  if (newTask) {
    formData = {
      ...
      cmb2: {
        taskbox_rest_metabox: {
          taskbook_prediction: document.querySelector(
            "textarea[name=prediction]"
          ).value,
          taskbook_pre_level: document.querySelector(
            "input[name=pre-level]:checked"
          ).value,
        },
      },
    };
    ...
    formData = {
      cmb2: {
        taskbox_rest_metabox: {
          taskbook_outcome: document.querySelector("textarea[name=outcome]")
            .value,
          taskbook_post_level: document.querySelector(
            "input[name=post-level]:checked"
          ).value,
        },
      },
    };
